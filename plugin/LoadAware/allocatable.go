...
import (
	...
	"Load-Aware-Scheduler-Plugin/apis/config"
...
func NewAllocatable(allocArgs runtime.Object, h framework.Handle) (framework.Plugin, error) {
	...
	if err != nil {
		klog.ErrorS(err, "Failed to get in-cluster config for metrics client")
	}
	...
	if kubeconfig != nil {
		...
		klog.Infof("Metrics client created")
	} else {
		klog.Infof("No metrics client created")
		metricsClient = nil
	}
	...
	klog.Infof("NodeResourcesAllocatable initialization successful!")
	return &Allocatable{
		handle: h,
...
func resourceScorer(resToWeightMap resourceToWeightMap, mode config.ModeType) func(resourceToValueMap, resourceToValueMap) int64 {
	return func(cost, allocable resourceToValueMap) int64 {
		// TODO: consider volumes in scoring.
		var nodeScore, weightSum int64
		for resource, weight := range resToWeightMap {
			resourceScore := score(allocable[resource]-cost[resource], mode)
